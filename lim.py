...
async def fetch_task(session: aiohttp.ClientSession, token: str) -> Tuple[dict, bool]:
    ...
    try:
        ...
        # URL diubah untuk mengambil tugas
        async with session.post("https://nebulai.network/opencompute/task", json={}, headers=headers, timeout=10) as resp:
...
async def submit_results(session: aiohttp.ClientSession, token: str, r1: float, r2: float, task_id: str) -> bool:
    ...
    try:
        ...
        # URL diubah untuk mengirim hasil
        async with session.post("https://nebulai.network/opencompute/submit", json=payload, headers=headers, timeout=10) as resp:
            data = await resp.json()

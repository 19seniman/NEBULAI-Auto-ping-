...
async def fetch_task(session: aiohttp.ClientSession, token: str) -> Tuple[dict, bool]:
    ...
    try:
        ...
        # URL diperbarui
        async with session.post("https://nebulai.network/opencompute/worker-node", json={}, headers=headers, timeout=10) as resp:
...
async def submit_results(session: aiohttp.ClientSession, token: str, r1: float, r2: float, task_id: str) -> bool:
    ...
    try:
        ...
        # URL diperbarui
        async with session.post("https://nebulai.network/opencompute/worker-node", json=payload, headers=headers, timeout=10) as resp:
            data = await resp.json()
